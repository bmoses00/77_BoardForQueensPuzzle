...
public class BoardForQueensPuzzle {
    ...
    public BoardForQueensPuzzle(int ranks) {
        ...
        // to prevent toString placing Q in first tile
    ...
    public void print(int[] input) {
        String output = "[";
        for (int i : input) output += i + ",";
        System.out.println(output + "]");
    }

    public boolean lastIsNg() {
        return lastIsNotGood(lastRankFilled);
    }
    public boolean lastIsNotGood(int fileToCheck) { // probably could be improved
        if (lastRankFilled == -1) return false;
        int tileWithQueen = filesWithQueens[fileToCheck];
        int invalidTile;

        for (int i = 0; i < lastRankFilled; i++) {
            // columns
            invalidTile = filesWithQueens[i];
            if (invalidTile == tileWithQueen) return true;
            // diagonal
            invalidTile = i + filesWithQueens[i] - lastRankFilled;
            if (invalidTile < ranks() - 1 && invalidTile >= 0)
                if (invalidTile == tileWithQueen) return true;
            // other diagonal
            invalidTile = lastRankFilled - i + filesWithQueens[i];
            if (invalidTile < ranks() - 1 && invalidTile >= 0)
                if (invalidTile == tileWithQueen) return true;
        }
        return false;
     ...
     */
     ...
     // not sure how to incorporate into lastIsNg
    public boolean accept() {
        return !lastIsNotGood(ranks() - 1);
    }
